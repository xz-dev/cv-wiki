...
**后续演进**:
此 PR 是保守方案。PR #1536 提供了最终解决大分辨率分配的机制。

---

### 案例5: 超高分辨率多块连续内存分配 (#1536) - 核心突破

**问题描述**:
在虚拟环境配置 8K、24K 等超大分辨率时，驱动尝试使用单块大的连续系统内存 (`new BYTE[size]`) 作为帧缓冲。随着系统运行，物理内存碎片化导致分配极易失败。此外，原机制为每页生成一个 SGList entry，当分辨率过大时会轻易突破 QEMU 的 16384 entry 限制，导致驱动或虚拟机崩溃。

**问题分析**:

1. **单块内存碎片化**: 长时间运行的 Windows 系统难以满足单块数百 MB 的连续物理内存分配请求。
2. **SGList 上限**:
   ```c
   // 原逻辑为每 4KB 页面分配一个条目
   // 对于 8K 帧缓冲 (127MB)，需要 ~32000 个条目，远超 QEMU 的 16384 限制
   ```
3. **BAR 内存限制**: Virtio-VGA 模式下 BAR0 被限制为 8MB，无法用于大分辨率帧缓冲。

**解决方案架构**:

彻底重构内存分配与描述符传递机制，实现带有一阶级降级策略（Fallback）的**多块连续物理内存分配**，并启用 **间接描述符 (Indirect Descriptors)**。

```cpp
// 1. 梯级块大小尝试 (Fallback Strategy)
const SIZE_T blockSizes[] = { 
    1024 * 1024,  // 1MB - 优先分配，对齐 Windows Segment Heap
    64 * 1024,    // 64KB
    32 * 1024,    // 32KB
    16 * 1024,    // 16KB
    4 * 1024      // 4KB
...
// 2. 多块内存分配核心逻辑
NTSTATUS VioGpuMemSegment::AllocateMultiBlock(SIZE_T size) {
    SIZE_T allocated = 0;
    while (allocated < size) {
        SIZE_T toAllocate = size - allocated;
        ...
        PVOID block = NULL;
        for (auto currentSize : blockSizes) {
            if (toAllocate >= currentSize) {
                block = MmAllocateContiguousMemory(currentSize, maxAddr);
                if (block) {
                    m_Blocks.push_back({block, currentSize});
                    allocated += currentSize;
                    break;
                }
            }
        }
    ...
    // 3. 构建 MDL 映射到连续虚拟地址
    return MapBlocksToVirtualAddress();
}

// 4. 启用间接描述符支持超大请求
void VioGpuQueue::AckFeature(UINT64 Feature) {
    if (Feature & VIRTIO_RING_F_INDIRECT_DESC) {
        m_bIndirectDescSupported = TRUE;
    }
}
```
...
1. **规避 2MB 碎片化悬崖**: 为什么首选 1MB？
   - 匹配 Windows Segment Heap（系统进程默认堆）的后端分配粒度。
   - 刚好避开 x64 上 2MB large page 对齐带来的严重物理内存碎片化失败问题。
   - 256 页的 2 次幂符合底层物理页分配器（如 buddy system）合并特性的最优大小。
2. **极大减少 QEMU SGList 条目数**: 
   将基于"页"的物理地址提取转为基于"块"。如果分配了 128 个 1MB 块，传给 QEMU 的就是 128 个条目，而不是 32768 个 4KB 页面条目，一举解决了 16384 的硬性限制。
3. **引入间接描述符协议**: `VIRTIO_RING_F_INDIRECT_DESC` 允许在单一 VirtQueue 请求内嵌入一个指向另一个缓冲区的指针，从而打破了单个 virtqueue 环大小的限制。

**后续演进**:
这个方案取代了之前的 PR #1479 (`viogpu-dynamic-segment-resize`) 动态重设大小方案，因为在 Windows 显示驱动模型（WDDM）的 `CommitVidPn` 阶段如果动态分配失败，根本不存在干净的 rollback 机制。而多块连续分配方案可以在初始化阶段安全完成。

---

### 案例6: 初始分辨率同步增强 (#1537) - ✅ 已合并 2026-03-15

**问题描述**:
虚拟机用户登录 Windows 后，`viogpuap.exe` 代理程序不会自动同步主机窗口的分辨率，必须由用户手动拉伸一次 spice 客户端（如 virt-manager）窗口才会触发分辨率对齐。

**原因**:
负责登录界面显示的 `vgpusrv` (运行于 Session 0) 受限于安全桌面隔离机制，不能直接改变用户交互会话 (Session 1+) 的分辨率。当 `viogpuap.exe` 在用户会话拉起时，只监听宿主机的 config-change 事件，而忽略了初始状态。

**解决方案**:
让 `viogpuap.exe` 启动时直接主动向设备查询当前的 host 窗口大小，并立即请求更改 Windows 分辨率，消除了长期存在的必须手动 resize 窗口的麻烦。
